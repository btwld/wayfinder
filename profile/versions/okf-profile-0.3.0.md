...
**Version 0.3.0** — profiles **OKF 0.2**
...
requests, and Linear records, are linked from concepts — ordinarily with
`Specified by` or `Implemented by` — and MUST NOT be mirrored into the bundle.
Their state is read from the tracker, never copied into frontmatter or body.
