...
A Profiled Bundle MUST express graph relationships only through the mechanisms OKF
defines. Markdown links, including links in a labelled Relationships bullet, remain
ordinary body links; a label is body context and MUST NOT be stored as graph
enrichment. Provenance through a `sources[].resource` that points to another concept
remains ordinary OKF provenance. Registry rows MUST remain table rows and MUST NOT
be represented as Profile-only graph nodes or edges in the bundle.
...
Any graph projection MUST be completely rebuildable from the bundle. Nothing may
