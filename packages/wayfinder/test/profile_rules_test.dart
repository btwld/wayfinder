...
void main() {
  ...
  test('validates concept metadata through text and JSON output', () async {
    final jsonResult = await runCli(<String>[
      'validate',
      '--output',
      'json',
      fixture('invalid-concepts'),
    ]);
  ...
  test(
    'validates registries, sources, and relationships deterministically',
    () async {
  ...
  test(
    'keeps concept advisories and absence boundaries gate-neutral',
    () async {
      final result = await runCli(<String>[
        'validate',
        '--output',
        'json',
        fixture('advisory-concepts'),
      ]);
  ...
  test('reports source paths that resolve to nothing as advisories', () async {
    ...
    final profile = output['profile']! as Map<String, Object?>;
    expect(profile['state'], 'PASS');
    final unresolved = findingSummary(
      profile,
  ...
  test('flags non-index markdown inside a references raw/ tier', () async {
    ...
    expect(
      findingSummary(profile),
      contains(
        'error concepta-profile/raw-directory-markdown '
  ...
  test('flags a raw/ tier sitting directly under references/', () async {
    ...
    expect(
      findingSummary(profile),
      contains('error concepta-profile/raw-directory-placement references/raw'),
    );
    expect(
      findingSummary(profile).where((line) => line.startsWith('error')),
      hasLength(1),
  ...
  test(
    'accepts presentation-equivalent indexes and contextual boundaries',
    () async {
      final result = await runCli(<String>[
        'validate',
        '--output',
  ...
  test(
    ...
    () async {
      for (final name in <String>['invalid-structure', 'structure-boundary']) {
        ...
        final original = await runCli(<String>[
          'validate',
          '--output',
          'json',
          source.path,
        ]);
        final reordered = await runCli(<String>[
          'validate',
          '--output',
          'json',
          reversed.path,
  ...
  test(
    'validates declaration and registry identities and table columns',
    () async {
      final result = await runCli(<String>[
        'validate',
        '--output',
        'json',
        fixture('invalid-registries'),
      ]);
  ...
  test('requires the actor registry only when actor fields are used', () async {
    ...
    expect(
      findingSummary(profile),
      contains('error concepta-profile/actor-registry-required actors.md'),
    );
  });
