...
Future<void> main(List<String> args) async {
  final parser = ArgParser()
    ..addOption('output', defaultsTo: 'build/embeddings')
  ...
  try {
    ...
    if (options.rest.isNotEmpty || outputPath.trim().isEmpty) {
      throw ArgumentError(
        'Usage: build_embeddings.dart [--output=DIR] [--offline]',
      );
    }
    ...
    final libraryName = Platform.isWindows
        ...
        ? 'libobjectbox.dylib'
        : 'libobjectbox.so';
    final library = File(p.join('lib', libraryName));
    if (!await library.exists()) {
      throw FileSystemException(
        'Run melos run objectbox:install before building.',
        library.path,
      );
    }
