...
- [Package guide](../packages/knowledge_embeddings/README.md): APIs, supported
  command-line entry points, fixtures and native setup.
- [Evaluation runbook](knowledge_embeddings_eval.md): deterministic gates,
  native checks and commands to reproduce comparisons.
- [Local implementation report](knowledge_embeddings_local_search.md): model
...
The current adapter still needs an in-memory knowledge snapshot before search.
The saved-vector reopen benchmarks reload sources and synchronize again; they
do not implement a standalone CLI that opens a complete saved snapshot. `okfp`
currently exposes validation only. Station's `validate`, `index` and `search`
commands are follow-up application work.

## Which experiment answers which question?

| Evidence | Question | Scope and limits |
| --- | --- | --- |
| [Initial implementation](knowledge_embeddings_local_search.md) | Does the local provider work, and what does storage cost? | 65 queries over mixed code/text; generic ObjectBox search uses HNSW |
| [Knowledge diagnostics](knowledge_embeddings_knowledge_retrieval.md) | How do scope, lifecycle, authority and negation affect lookup? | 13 caller-projected cases; diagnoses failures without defining OKF rules |
| [OKF component experiment](knowledge_embeddings_ablation.md) | What do headings, lifecycle and explicit relationship policies contribute? | 35 questions parsed through OKF; exact eligible-vector scoring |
| [Retrieval comparison](knowledge_embeddings_comparison.md) | What do keyword, BM25, embeddings and hybrid cost and recover? | 160 synthetic questions split by topic; isolated processes and separate storage scale test |
| [Model comparison](knowledge_embeddings_model_comparison.md) | Does a different small model improve on XS? | Five artifacts on the same previously inspected fixture; exploratory comparison |

The [executed benchmark plan](knowledge_embeddings_benchmark_plan.md) records
the protocol, and the [fixture guide](../packages/knowledge_embeddings/fixtures/README.md)
