...
| --- | --- |
| Profile index generation and MCP writes | Guide §3 has a generator contract; the current `okfp` CLI implements validation only. [#45](https://github.com/conceptadev/okf-profile/issues/45) proposes Profile-aware MCP writes and identifies the projection/transaction design choice. Resolve that seam before adding write commands. |
| Installation and distribution | [#53](https://github.com/conceptadev/okf-profile/issues/53) and [PR #54](https://github.com/conceptadev/okf-profile/pull/54) cover installation without Dart, plugin packaging, and release distribution. Continue that work there; the housekeeping change does not add a competing installer or manifest. |
| One consumer validation entry point | [#52](https://github.com/conceptadev/okf-profile/issues/52) asks about format/index checks alongside validation. The existing semantic gate differs from a generic index generator's rendering. Do not advertise a generic reindex as satisfying Profile §9 without validating the resulting projection. |
| Generic Profile platform | [#17](https://github.com/conceptadev/okf-profile/issues/17) remains an exploration with explicit evidence triggers. A second-brain workflow does not by itself justify a generic provider platform. |
