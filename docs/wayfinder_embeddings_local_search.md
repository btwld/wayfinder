...
The subsequent [knowledge retrieval review](wayfinder_embeddings_knowledge_retrieval.md)
...
[retrieval comparison](wayfinder_embeddings_comparison.md) and
[model comparison](wayfinder_embeddings_model_comparison.md) provide repeated
process measurements and the evidence behind [ADR-0009](adr/0009-local-knowledge-retrieval.md).
See the [documentation guide](wayfinder_embeddings.md) for the experiment map.
...
[local baseline](../packages/wayfinder_embeddings/fixtures/benchmarks/local_metrics_baseline.json)
...
[evaluation runbook](wayfinder_embeddings_eval.md):

```bash
# From packages/wayfinder_embeddings; ObjectBox must already be installed.
