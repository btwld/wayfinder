...
through this artifact and the separate coverage matrix. This review has 77
...
| §13 requires graph consumers to keep Markdown edges untyped and prohibits Profile-only nodes or edges from relationship labels and registry rows | §6.1 defines links as untyped edges; §7 actor fields remain strings | Pass | The rule preserves the ordinary OKF graph contract exactly and prevents the structural registries from becoming a second graph schema. |
| §13 requires every graph view to rebuild completely from the bundle and prohibits graph-only state | §2 defines concepts and links in the bundle; §5.1 and §6 define provenance and link relationships | Pass | Rebuildability makes the graph a disposable consumer view and leaves the Markdown bundle and OKF mechanisms authoritative. |
...
| Profile rule | OKF basis | Compatibility result | Reasoning |
| --- | --- | --- | --- |
| §15.3 keeps a 2026.1 bundle under its declared release until migration completes, then changes only the Profile selector while retaining OKF 0.2 | §12 permits the root `okf_version` declaration | Pass | The migration preserves the upstream version and changes only body prose in an ordinary declaration concept. |
| §15.3 regenerates every index to §9 semantics and removes index-only descriptions and authored ordering | §8 permits generated grouped indexes | Pass | The result remains ordinary OKF index Markdown and retains upstream entry meanings. |
| §15.3 normalizes root-log lead-word shape while preserving authored history | §9 defines newest-first dated log prose and conventional lead words | Pass | Migration selects a permitted shape without deriving or replacing the historical account. |
| §15.3 adds the conditional actor registry when needed and permits an unnecessary existing registry to remain | §§3.1 and 4 make the registry an ordinary concept | Pass | Presence changes only Profile producer policy; actor strings and OKF conformance stay unchanged. |
| §15.3 adds truthful baseline metadata and the complete type vocabulary with registered extensions | §4.1 defines the fields and open type vocabulary | Pass | Migration populates existing OKF fields and body tables while preserving unknown-type tolerance. |
| §15.3 normalizes actor sides and periods and uses `unknown` where evidence is absent | §§4.2 and 7 permit body lookup tables while actor IDs stay strings | Pass | Organizational history remains Profile body data and never changes OKF actor or trust semantics. |
| §15.3 adjudicates producer fields, semantic tag aliases, and materially derived claims before normalization | §§4.1 and 5.1 define tolerant fields, tags, sources, and attribution | Pass | Information moves only to an applicable upstream mechanism or body prose; consumers continue preserving unknown input. |
| §15.3 reviews event-derived concepts and Interaction Records against durable capture | §§2 and 4 permit ordinary producer concepts and free bodies | Pass | Migration narrows retained Concepta knowledge without changing how generic consumers parse it. |
| §15.3 applies promotion and splitting guidance while allowing legitimate reviewed exceptions | §§2 and 5 scope metadata to ordinary concepts | Pass | Boundary choices use more or fewer normal OKF concepts and add no sub-concept schema. |
| §15.3 prohibits inferring generation, verification, freshness, affiliation, or provenance | §5 defines truthful optional trust, lifecycle, and provenance signals | Pass | The prohibition preserves meaningful absence and upstream semantics. |
| §15.3 reshapes labelled relationships and recommends documenting project labels | §§4.2 and 6.1 keep relationship kind in body prose around ordinary links | Pass | Migration changes presentation context without creating typed edges or a closed label vocabulary. |
| §15.3 chooses one specification owner and reviews paths and moves against known citations | §§2, 3, and 6 define concept paths and external links without repository workflow state | Pass | Ownership and coordinated repair constrain producer storage while IDs and links remain ordinary OKF constructs. |
| §15.3 deprecates retained stable superseded concepts and links available successors | §§5.4 and 6.1 define `deprecated` and ordinary links | Pass | Retirement uses the upstream lifecycle value and graph edge unchanged. |
| §15.3 reviews and repairs mirrored material without weakening followable resources | §§5.1 and 6.3 define sources, scope descriptors, and `references/` | Pass | Migration preserves the concrete-versus-scope distinction and keeps media policy outside OKF meaning. |
| §15.3 requires no repair for conventions 2026.2 relaxes | §11 preserves tolerant consumption | Pass | Removing local restrictions cannot make a valid OKF bundle less readable or alter its independent result. |
